...
`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		input, errDetails, hasErr := domain.ParseArgs(args, moduleName, false)
		if hasErr {
		...
		if err := models.GenerateModel(input); err != nil {
			log.Error(err)
			return
		}
		if err := gormimpl.GenerateRepository(input); err != nil {
			log.Error(err)
			return
		}
		if err := echoimpl.GenerateController(input); err != nil {
			log.Error(err)
			return
		}
...
🏁 Next steps:
	...
	➡️ Add dbConn = connections.GetGormConn(conf.DatabaseConnectionConfig)
