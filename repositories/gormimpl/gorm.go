...
func GenerateRepository(input parser.GenerateModelInput, goDeps deps.Container) error {
	...
	if err := doGenerateRepositoryTest(input, data, goDeps, modelPackagePath); err != nil {
...
func doGenerateRepositoryTest(input parser.GenerateModelInput, generatorData generatorData, goDeps deps.Container, modelPackagePath string) error {
	...
	suiteFile.
		Id("import").
		Parens(
			Id(goDeps.Std.Testing.Alias).Lit(goDeps.Std.Testing.Path).
				Line().
				Id(goDeps.Project.Core.Connections.Alias).Lit(goDeps.Project.Core.Connections.Path).
				Line().
				Id(goDeps.Project.Pkg.TestFunc.Alias).Lit(goDeps.Project.Pkg.TestFunc.Path).
				Line().
				Id(goDeps.Ginkgo.Alias).Lit(goDeps.Ginkgo.Path).
				Line().
				Id(goDeps.Gomega.Alias).Lit(goDeps.Gomega.Path).
				Line(),
		).
		Line().
		Line().
